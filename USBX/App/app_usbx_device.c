...
UINT MX_USBX_Device_Init(VOID *memory_ptr)
{
  ...
  /* USER CODE BEGIN MX_USBX_Device_Init */
   ...
   tx_status = tx_thread_create(&ux_cdc_read_thread, "cdc_acm_read_usbx_app_thread_entry",
                                ...
                                pointer, USBX_APP_STACK_SIZE, 5, 5, TX_NO_TIME_SLICE,
                                TX_AUTO_START);
   ...
   tx_status = tx_thread_create(&ux_cdc_write_thread,
                                ...
                                pointer, USBX_APP_STACK_SIZE, 5, 5,
                                TX_NO_TIME_SLICE,
                                TX_AUTO_START);
   ...
   if(tx_queue_create(&usb_send_queue, "usb send queue",1,pointer,100*4)!=TX_SUCCESS)
   {
	 ret = TX_GROUP_ERROR;
   }
...
void usbx_app_thread_entry(ULONG arg)
{
  /*Delay to let other parts initialize first*/
  tx_thread_sleep(10);
